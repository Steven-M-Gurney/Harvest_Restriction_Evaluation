...
# This code reads in, formats, and combines count and camera-location data; and addresses missing data.
# Location data is private to Michigan State University.

# Load packages
library(dplyr)
library(anytime) # To format time data.
library(reshape2) # To reshape missing data.
...
df$SpeciesID[df$CommonName=="Coyote"] <- 4
df$SpeciesID[df$CommonName=="Fox"] <- 5
df$SpeciesID[df$CommonName=="Camera Problem"] <- 6
df$SpeciesID[df$CommonName=="Doe"] <- 7
df$SpeciesID[df$CommonName=="Fawn"] <- 8
df$SpeciesID[df$CommonName=="Legal Buck"] <- 9
df$SpeciesID[df$CommonName=="Sub-legal Buck"] <- 10
df$SpeciesID[df$CommonName=="Camera Setup/Takedown"] <- 12
df$SpeciesID[df$CommonName=="Remove"] <- 13

# Double check to see if species counts changed.
df %>% count(SpeciesID)

# Save output
write.csv(df, "2022CombinedQryFlatFileVerified.txt")

# Format time data for all verified photos.
df$ImageDate2 <- as.POSIXct(df$ImageDate, format="%m/%d/%Y %H:%M:%S")
...
df2 <- merge(df,cam_locs, by.x="LocationName", by.y="siteID", all.x=TRUE)
...
df3 <- re.time(df2$ImageDate2, df2$ycoord, df2$xcoord, -4, df2)
...
df3 <- subset(df3, select=-c(xcoord,ycoord))
...
df3$SpeciesID[ df3$DetailText == "Antlered" ] <- "20" # Unknown Antlered SpeciesID now = "20"
df3$SpeciesID[ df3$DetailText == "Antlerless" ] <- "30" # Unknown Antlerless SpeciesID now = "30"
...
sherman <- subset(df3,LocationName < 12000)
tyrone <- subset(df3,LocationName > 11999.5 & LocationName <13000)
bowne <- subset(df3,LocationName > 12999.5 & LocationName <14000)
nelson <- subset(df3,LocationName > 13999.5 & LocationName <15000)
...
sheridan <- subset(df3,LocationName > 20000 & LocationName <22000)
easton <- subset(df3,LocationName > 21999.5 & LocationName <23000)
bushnell <- subset(df3,LocationName > 22999.5 & LocationName <24000)
douglass <- subset(df3,LocationName > 23999.5)
