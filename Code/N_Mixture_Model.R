# This script can be used for each sex-and-age class individually, but here it includes
# an example for legal-antlered deer. With regard to other sex-and-age classes, note that 
# some hardcoding is needed when reading in count data and saving output.
...
# Use index position of numbers in LocationName to extract more identifiers.
# Specifically, treatment (1 = NonAPR; 2 = APR),township pairing (4 pairs total), and township site
...
count19 <- read_csv("LegalCounts_2019.csv") |> 
  add_column( year = 2019) |> 
  mutate( trt = substr( LocationName, 1, 1),
          twn = substr( LocationName, 2, 2), 
          site = substr( LocationName, 3, 5)) |> 
...
count20 <- read_csv("LegalCounts_2020.csv") |> 
  add_column( year = 2020) |> 
  mutate( trt = substr( LocationName, 1, 1),
          twn = substr( LocationName, 2, 2), 
          site = substr( LocationName, 3, 5)) |> 
...
count21 <- read_csv("LegalCounts_2021.csv") |> 
  add_column( year = 2021) |> 
  mutate( trt = substr( LocationName, 1, 1),
          twn = substr( LocationName, 2, 2), 
          site = substr( LocationName, 3, 5)) |> 
...
count22 <- read_csv("LegalCounts_2022.csv") |> 
  add_column( year = 2022) |> 
  mutate( trt = substr( LocationName, 1, 1),
          twn = substr( LocationName, 2, 2), 
          site = substr( LocationName, 3, 5)) |> 
...
# name new column "rep" for repeated visits, and add associated values to the new "count"
# column). 
# Drop non-numeric characters from new "rep" column. Arrange year, siteID,
# and rep in ascending order.
...
# count (rep) "j", each site (siteID) "i" (number of sites depends on year;
# addressed above and incorporated below), and assign counts.
for( t in 1:4 ) {
  for( j in 1:9) {
    for( i in 1:nsite[t]){
...
#save(out, code, data, constants, file = "Results_LegalNmix_Twp_ME.RData")
...
#write.csv(deer.me, "legal.trt.me.csv",row.names = FALSE)
...
#write.csv(deer.diff, "legal.trt.diff.csv", row.names = FALSE)
